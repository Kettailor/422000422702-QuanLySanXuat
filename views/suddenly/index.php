...
.summary-cards p { margin: 0; color: #6c757d; font-weight: 500; }
...
/* ===== CARD BIÊN BẢN (NHỎ GỌN GIỐNG 3 CARD TRÊN) ===== */
.summary-cards.mt-3 {
  display: grid;
  grid-template-columns: repeat(auto-fit, minmax(200px, 1fr));
  gap: 20px;
...
.card-type {
  ...
  transition: 0.2s;
  cursor: pointer;
  height: 140px; /* Chiều cao giống 3 card trên */
  display: flex;
...
/* Màu riêng từng loại */
.card-production { border-top-color: #fd7e14; }
.card-worker { border-top-color: #4277c1ff; }
...
.card-production .card-count { color: #fd7e14; }
.card-worker .card-count { color: #4277c1ff; }
...
.badge-type.production { background-color: #fd7e14; }
.badge-type.worker { background-color: #4277c1ff; }
...
.badge-result.success{ background:#28a745; }
.badge-result.failed{ background:#dc3545; }
.badge-result.unchecked{ background:#6c757d; color:#fff; }
...
<div class="qa-container">
  ...
  <a href="?controller=suddenly&action=create&type=production" 
     class="card card-type card-production">
    <div class="card-body">
  ...
  <a href="?controller=suddenly&action=create&type=worker" 
     class="card card-type card-worker">
    <div class="card-body">
      <div class="card-header d-flex align-items-center justify-content-center mb-2">
      ...
      <table class="table table-hover align-middle text-nowrap" id="lotTable">
        ...
        <tbody>
          ...
          <tr data-type="<?= $typeGroup ?>" data-status="<?= $status ?>">
            ...
            <td class="text-end">
  <a href="?controller=suddenly&action=read&id=<?= urlencode($r['IdBienBanDanhGiaDX']) ?>" 
...
<script>
// Lọc theo loại biên bản (production / worker)
...
function filterByResult(type) {
  ...
  rows.forEach(row => {
    ...
    if (type === 'all') {
      ...
      row.classList.toggle('hidden-row', status !== type);
    }
  });
}

// Xử lý nút XÓA biên bản
document.querySelectorAll('.btn-delete').forEach(btn => {
  btn.addEventListener('click', () => {
    const id = btn.dataset.id;
    if (confirm(`Bạn có chắc muốn xóa biên bản ${id} không?`)) {
      // Gửi yêu cầu xóa (AJAX hoặc giả lập)
      fetch(`?controller=suddenly&action=delete&id=${encodeURIComponent(id)}`, {
        method: 'GET'
      })
      .then(res => {
        if (res.ok) {
          ...
          alert('Không thể xóa biên bản. Vui lòng thử lại.');
        }
      })
      .catch(() => alert('Lỗi kết nối khi xóa biên bản.'));
    }
  });
});
